...
if (JSON.parse(localStorage.getItem(loginedUserName.slice(5, -2)))) {
    ...
    let DBCategoryHrefs = categoryArray.map((category) => category.getAttribute("href"));
    let DBCategoryTexts = categoryArray.map((category) => category.textContent);
    let LSCategoryHrefs = JSON.parse(localStorage.getItem(loginedUserName.slice(5, -2))).href;
    let LSCategoryTexts = JSON.parse(localStorage.getItem(loginedUserName.slice(5, -2))).text;
    if (JSON.stringify(DBCategoryHrefs.sort()).length >
        ...
        // ローカルストレージの更新
        localStorage.setItem(loginedUserName.slice(5, -2), JSON.stringify(savedCategories));
    }
    else if (JSON.stringify(DBCategoryHrefs.sort()).length <
        JSON.stringify(LSCategoryHrefs.sort()).length) {
        ...
        let newLSCategoryTexts = LSCategoryTexts.filter((text) => DBCategoryTexts.includes(text));
        //　LSのカテゴリーの要素順序となるDBのインデックス番号を取得
        let indicesFromDBCategoryTexts = getIndicesFromBtoA(DBCategoryTexts, newLSCategoryTexts);
        //　上記のインデックス番号に対応するDBのhrefを取得
        let hrefsFromDBCategoryHrefs = getValuesFromIndices(DBCategoryHrefs, indicesFromDBCategoryTexts);
        savedCategories = {
    ...
    else if (JSON.stringify(DBCategoryTexts.sort()) !==
        ...
        let LSCategoryTexts = JSON.parse(localStorage.getItem(loginedUserName.slice(5, -2))).text;
        // 更新前のカテゴリー名を取得
        let beforeUpdateText = LSCategoryTexts.filter((text) => !DBCategoryTexts.includes(text));
        // 更新されたカテゴリー名に対応するLSのインデックス番号を取得
        let updatedLSIndex = LSCategoryTexts.indexOf(beforeUpdateText[0]); // 更新は1個ずつしか行わないことを前提に[0]を記載
        // 更新後のカテゴリー名を取得
        let afterUpdateText = DBCategoryTexts.filter((text) => !LSCategoryTexts.includes(text));
        // LSのカテゴリー名を更新
        ...
        // DBとLSの情報が同一の場合
        savedCategories = JSON.parse(localStorage.getItem(loginedUserName.slice(5, -2)));
    }
}
else {
    // LSにデータが存在しない場合
